...
class GroupedLineChartComponent extends AbstractChart<any, any> {
		render() {
			...
			// Fallback seguro para dados vazios ou inválidos
			const allData = Array.isArray(datasets)
				? datasets.flatMap((ds: any) =>
						Array.isArray(ds.data)
							? ds.data.filter((v: any) => typeof v === 'number' && isFinite(v) && !isNaN(v))
							: []
					)
				: [];
			const maxValue = allData.length > 0 ? Math.max(...allData) : 1;
			const minValue = allData.length > 0 ? Math.min(...allData) : 0;
			const totalPoints = Array.isArray(labels) ? labels.length : 0;
			const dynamicWidth = padding * 2 + Math.max(0, (totalPoints - 1)) * 60;

			const getPoint = (i: number, value: number) => {
				const x = padding + i * 60;
				const y = chartHeight - (maxValue - minValue === 0 ? 0 : ((value - minValue) / (maxValue - minValue)) * chartHeight) + 10;
				return { x, y };
			};
				...
				<Svg width={40} height={chartHeight + 40}>
					{[0, 0.25, 0.5, 0.75, 1].map((p, idx) => (
						<SvgText
							...
							textAnchor="start"
						>
							{Math.round(minValue + (maxValue - minValue) * p) || 0}
						</SvgText>
					))}
				</Svg>
...
const GroupedLineChartUser = (props: any) => {
					...
					<ScrollView style={{ maxHeight: 200 }}>
						...
						{tooltip.datasets
							...
							<View key={idx} style={{ flexDirection: 'row', alignItems: 'center', marginBottom: 4 }}>
								...
								<Text style={{ color: '#fff', fontSize: 15 }}>{props.profiles.find((f: any) => f.id == ds.label)?.name || `Executor ${idx + 1}`}: {tooltip.values[idx]}%</Text>
							</View>
						))}
					</ScrollView>
				</View>
...
export default GroupedLineChartUser;