				...
				<Tab.Screen
					...
					options={{
						...
						tabBarLabel: ({ color, focused }) =>
							focused ? '' : <Text style={{ color, fontSize: 10 }}>Registrar</Text>,
						tabBarIcon: ({ color, size, focused }) => (
							<View
								className={`${focused ? 'h-[56px] w-[56px] items-center justify-center' : 'bg-transparent'} rounded-full p-2`}
								style={{ backgroundColor: focused ? color : 'transparent' }}
							>
								<Plus color={focused ? '#FFFFFF' : color} size={size} />
							</View>
						),
						header: TabsHeader,
					}}
