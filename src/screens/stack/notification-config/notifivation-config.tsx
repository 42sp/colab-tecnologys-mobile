...
const NotificationConfig = () => {
	...
	const scheduleDailyNotification = async () => {
		...
		const existingNotification = scheduledNotifications.find((f: any) => {
			if (Platform.OS === 'ios') {
				...
				return f.trigger?.hour === date.getHours() && f.trigger?.minute === date.getMinutes()
			}
		})
		...
		if (role.role_name === 'encarregado')
			body =
				'Existem serviços aguardando sua aprovação. Por favor, revise e aprove as atividades pendentes.'
		else if (role.role_name === 'executor')
			body =
				'Você tem tarefas atribuídas para hoje. Por favor, verifique suas atividades e mantenha o progresso em dia.'
		else body = 'Você tem uma notificação diária agendada.'
	...
	const getScheduledNotifications = async () => {
		const notificacoes: Notifications.NotificationRequest[] =
			await Notifications.getAllScheduledNotificationsAsync()
		setScheduledNotifications(notificacoes)
	}

	return (
		<SafeAreaView className="m-4 flex-1 items-start justify-start bg-gray-100">
			<View className="w-full gap-1 rounded-lg bg-white p-4">
				...
				<Controller
					...
					render={({ field: { onChange, value } }) => (
						<>
							{Platform.OS === 'android' && (
								<TouchableOpacity
									className="flex w-full flex-row items-center rounded-md border p-4 text-center"
									onPress={() => setShow(true)}
								>
									<Clock size={18} />
									<Text className="text-md ml-2 flex font-inter-medium">{value}</Text>
								</TouchableOpacity>
							)}
							{show && (
								<RNDateTimePicker
									...
									onChange={(event, selectedDate) =>
										handleTimeChange(event, selectedDate, onChange)
									}
								/>
							)}
						</>
				...
				<View className="my-5">
					<Text className="mt font-inter-medium text-lg">Notificações Agendadas:</Text>
					<FlatList
						className="max-h-80"
						data={[...scheduledNotifications].sort((a, b) => {
							const getHourMinute = (notif: any) => {
								...
								if (
									notif.trigger?.dateComponents?.hour !== undefined &&
									notif.trigger?.dateComponents?.minute !== undefined
								) {
									return (
										notif.trigger.dateComponents.hour * 60 + notif.trigger.dateComponents.minute
									)
								}
								return 0
							}
								...
								<View className="flex-row items-center">
									...
									<View>
										<Text className="mx-2 font-inter-medium">
											{item.trigger &&
											item.trigger.hour !== undefined &&
											item.trigger.minute !== undefined
												? `${item.trigger.hour}:${item.trigger.minute}`
												: item.trigger &&
													  item.trigger.dateComponents &&
													  item.trigger.dateComponents.hour !== undefined &&
													  item.trigger.dateComponents.minute !== undefined
				...
				<TouchableOpacity
					className="mt-4 w-full rounded-lg bg-black p-4"
					onPress={scheduleDailyNotification}
				>
					<Text className="text-center text-white">Agendar Notificação Diária</Text>
				</TouchableOpacity>
			</View>
		</SafeAreaView>
	)
